...
### Правило частичного обновления: решает наличие ключа

Внутри почтового блока **весь смысл несёт наличие ключа**, и состояний здесь три:

| что вы написали | что происходит |
|---|---|
| ключа нет | поле не отправляется, и реестр сохраняет то, что у него есть |
| ключ со значением | поле получает это значение |
| ключ со значением `''` | поле **очищается** — единственный способ убрать `org`, `sp` или `pc` |

```php
// Меняем город, очищаем организацию, имя и улицу оставляем ровно как есть.
$client->contact()->update('acme-01', [
    'chg' => ['postalInfo' => [
        'type' => 'int',
        'city' => 'Lviv',
...
**Указывайте `city` и `cc` всякий раз, когда трогаете адрес.** `<contact:addr>` — это схемная
последовательность, в которой оба обязательны, поэтому блок уходит целиком или не уходит вовсе: как
только вы упомянули `street`, `city`, `sp`, `pc` или `cc`, наружу отправляется весь блок, и `city` с
`cc` едут в нём независимо от того, задали вы их или нет. Не задать их здесь — значит отправить их
пустыми.
