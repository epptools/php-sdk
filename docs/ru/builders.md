...
### Внутри адреса решает наличие

Три состояния различаются, и именно ими выражается частичная смена адреса:

| Что вы пишете | Что происходит |
|---|---|
| не передаёте аргумент или передаёте `null` | поле не отправляется, и реестр сохраняет своё значение |
| передаёте значение | поле принимает это значение |
| передаёте `''` | поле **очищается** — единственный способ убрать `org`, `stateProvince` или `postalCode` |

```php
// Переселить контакт во Львов, очистить организацию, имя и улицу оставить как есть.
$client->contact()->updateBuilder('C-0001')
    ->changeInternationalAddress(city: 'Lviv', countryCode: 'UA', org: '')
    ->send();
```

**Передавайте `city` и `countryCode` всякий раз, когда трогаете адрес.** `<contact:addr>` — это
схемная последовательность, в которой оба поля обязательны, поэтому блок выдаётся целиком или не
выдаётся вовсе: как только вы упомянули `street`, `city`, `stateProvince`, `postalCode` или
`countryCode`, наружу уходит весь блок, а эти два поля едут с ним — передали вы их или нет. Не
передать их здесь значит отправить их пустыми.

Форма, которую вы не упомянули, — локальная или международная — остаётся нетронутой.
